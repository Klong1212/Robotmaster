...
class MazeExplorer:
    ...
    def _upsert_marker(self, label, mx, my, meta):
        ...
        if label not in self.marker_map or not self.marker_map[label]:
            self.marker_map.setdefault(label, []).append(((mx, my), {**meta, "count": 1}))
            return "created"

        best_i = -1
        best_d = 1e9
        for i, ((ex, ey), emeta) in enumerate(self.marker_map[label]):
            d = math.hypot(mx - ex, my - ey)
            if d <= r_grid:
                b_old = float(emeta.get("bearing_deg", bearing_new))
                if abs(_angle_diff_deg(bearing_new, b_old)) <= BEARING_GATING_DEG:
                    if d < best_d:
                        best_d = d
                        best_i = i
        ...
        cnt = int(emeta.get("count", 1)) + 1
        dist_old = float(emeta.get("dist_m", meta["dist_m"]))
        bear_old = float(emeta.get("bearing_deg", bearing_new))

        dist_new = _ema(dist_old, meta["dist_m"])
        delta = _angle_diff_deg(bearing_new, bear_old)
        bear_blend = _wrap_angle_deg(bear_old + EMA_ALPHA * delta)

        merged_meta = {**emeta}
        merged_meta.update({
            "dist_m": round(dist_new, 3),
            "bearing_deg": round(bear_blend, 2),
            "ts": meta["ts"],
        ...
        self.marker_map[label][best_i] = ((new_x, new_y), merged_meta)
        return "merged"
...
def plot_map_with_walls(graph, blocked, path, marker_map, filename="maze_map.png"):
    ...
    for name, hits in (marker_map or {}).items():
        for (mx, my), meta in hits:
            ...
            label = f"{name}"
            if "count" in meta:
                label += f" (n={meta['count']})"
            plt.text(mx + 0.06, my + 0.06, label, fontsize=8, zorder=7)
