        ...
        with self._lock:
            ...
            if vision_info:
                ...
                for t in vision_info:
                    ...
                    self.markers.append(str(label))  # <<< FIX: ย้ายเข้า loop ให้ถูกต้อง
...
class MazeExplorer:
        ...
        for scan_direction in range(4):
            ...
            if distance_mm >= WALL_THRESHOLD_MM:
                if neighbor_pos[0] >= 0 and neighbor_pos[1] >= 0 and neighbor_pos[0] < self.border[0] and neighbor_pos[1] < self.border[1]:
                    ...
                    print(f"           - Open path recorded at {neighbor_pos}.")  # <<< message ให้ตรง
                    continue
            else:
    ...
    def run_mission(self):
        ...
        while True:
            ...
            # <<< NEW: บังคับสแกนถ้าจุดปัจจุบันไม่มีเพื่อนบ้านในกราฟ (กันเคส resume แล้วจบทันที)
            need_scan = (
