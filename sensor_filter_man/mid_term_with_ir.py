...
class MazeExplorer:
    ...
    def scan_surroundings_with_gimbal(self, previous_position=None):
        ...
        # อ่านค่าจาก IR sensors (digital 0/1)
        try:
            ...
            print(f"Error reading IR sensors: {e}, assuming no side walls")
            wall_left = False
            wall_right = False
        ...
        for scan_direction in range(4):
            ...
            # เลือกเซ็นเซอร์ตามทิศที่สแกน relative to robot orientation
            relative_direction = (scan_direction - self.current_orientation + 4) % 4
            
            if relative_direction == 0:  # ด้านหน้าของหุ่น -> ใช้ ToF
            ...
            elif relative_direction == 1:  # ด้านขวาของหุ่น -> ใช้ IR ขวา
                has_wall = wall_right
                distance_mm = 100.0 if has_wall else 1000.0  # ค่าประมาณ
                print(f"         - IR Right: Wall={has_wall}")
                wall_distances[f'{scan_direction}'] = distance_mm
                
            elif relative_direction == 3:  # ด้านซ้ายของหุ่น -> ใช้ IR ซ้าย
                has_wall = wall_left
                distance_mm = 100.0 if has_wall else 1000.0  # ค่าประมาณ
                print(f"         - IR Left: Wall={has_wall}")
                wall_distances[f'{scan_direction}'] = distance_mm
            ...
            # Wall detected - เพิ่มเป็น blocked path (ไม่ต้องหา marker)
        ...
        # รีเซ็นเตอร์กิมบอล (ไม่จำเป็นแล้วเพราะไม่ได้ใช้)
        ...
        # autosave หลังสแกนหนึ่งจุด (ถ้ามีฟังก์ชัน)
        try:
            _autosave(self)
        except NameError:
            pass  # ถ้าไม่มี _autosave function ก็ข้าม
    ...
    def execute_path(self, path):
        ...
        for i in range(len(path) - 1):
            ...
            time.sleep(0.2)
            _autosave(self)  # autosave เมื่อถึงกริดใหม่
            print("end++++++++++++++++++++++++++++++++++++++++++++++++++")
