...
CAMERA_HORIZONTAL_FOV = 96.0
GIMBAL_SWEEP_OFFSETS = [0, -30, +30]  # กวาดหา marker 3 มุม (ศูนย์/ซ้าย/ขวา)
...
def _esc_listener():
    """กด ESC เพื่อหยุดการทำงานอย่างสุภาพ (ไม่บล็อก)"""
    global STOP_FLAG
    if _HAS_MSVCRT:
...
# ==============================================================================
# State save/load (ต่อภารกิจรอบหน้า)
# ==============================================================================
...
def save_map_state(explorer, filename=STATE_FILE):
    try:
        state = {
            ...
            # { name: [ {grid_pos:[gx,gy], wall:str, offset_m:float}, ... ] }
            "marker_map": {
                k: [
                    {"grid_pos": list(item.get("grid_pos", [0,0])),
                     "wall": item.get("wall", ""),
                     "offset_m": float(item.get("offset_m", 0.0))}
                    for item in v
                ]
...
def load_map_state(explorer, filename=STATE_FILE):
    ...
    try:
        ...
        for name, items in s.get("marker_map", {}).items():
            explorer.marker_map[name] = []
            for item in items:
                gp = tuple(item.get("grid_pos", [0, 0]))
                wall = item.get("wall", "")
                off = float(item.get("offset_m", 0.0))
                explorer.marker_map[name].append({"grid_pos": gp, "wall": wall, "offset_m": off})
...
class MazeExplorer:
    def __init__(self, ep_robot, tof_handler, vision_handler, gimbal_handler, pose_handler):
        ...
        self.step_counter = 0  # ใช้ trigger บางอย่างตามรอบเดิน
    ...
    # ==================== ตรรกะ/การเคลื่อนที่ (คงตามโค้ดอ้างอิง) ====================
    def _log_marker_precise(self, scan_direction: int, detections: list, tof_mm_at_frame: float):
        """
        detections: list of (label, x) จาก VisionDataHandler
        กฎ: first-hit-wins ต่อ (marker_name, grid, wall)
        ซ้ายในภาพ = ค่าบวก, ขวา = ค่าลบ (ผ่าน offset_m)
        """
        if not detections:
            return
        ...
        for (marker_name, x_coord) in detections:
            # ข้ามถ้าเคยบันทึกชื่อเดียวกันที่กริด/ผนังเดียวกันแล้ว
            if marker_name in self.marker_map:
                for ex in self.marker_map[marker_name]:
                    if ex.get("grid_pos") == self.current_position and ex.get("wall") == wall_name:
                        # พบซ้ำ — ข้ามรายการนี้
                        break
            ...
            already = any((f["grid_pos"] == finding["grid_pos"] and f["wall"] == finding["wall"])
    ...
    def scan_surroundings_with_gimbal(self, previous_position=None):
        ...
        for scan_direction in range(4):
            ...
            # ปรับเข้า/ออกให้ clearance ~0.20m เพื่อสแกนชัดขึ้น
            ...
            # สแกน marker ที่ yaw 0/-45/+45 (อิง angle_to_turn_gimbal)
            detected_any = False
            for delta in GIMBAL_SWEEP_OFFSETS:
                target_yaw = angle_to_turn_gimbal + delta
                self.ep_gimbal.moveto(yaw=target_yaw, pitch=-15,
                                      yaw_speed=GIMBAL_TURN_SPEED, pitch_speed=60
                                      ).wait_for_completed()
                time.sleep(0.5)
                tof_now = self.tof_handler.get_distance()
                detections = self.vision_handler.get_markers()
    ...
    def periodic_wall_clearance_adjust(self, target_clearance_m=0.18):
        ...
        """
        for scan_direction in range(4):
            print(f"   Clearance check direction: {ORIENTATIONS[scan_direction]}...")
            angle_to_turn_gimbal = (scan_direction - self.current_orientation) * 90
            if angle_to_turn_gimbal > 180: angle_to_turn_gimbal -= 360
            if angle_to_turn_gimbal < -180: angle_to_turn_gimbal += 360
    ...
    def move_forward_pid(self, distance_m, speed_limit=0.6):
        ...
        pid = PIDController(Kp=2, Ki=0.02, Kd=0.25, setpoint=distance_m, output_limits=(-speed_limit, speed_limit))
    ...
    def turn_pid(self, target_angle, speed_limit=90):
        ...
        pid = PIDController(Kp=2.5, Ki=0, Kd=0.5, setpoint=0, output_limits=(-speed_limit, speed_limit))
        while True:
    ...
    def run_mission(self):
        ...
        while True:
            ...
            # สแกนถ้าจุดนี้ยังไม่มีกราฟเพื่อนบ้าน (กัน resume แล้วไปต่อไม่ถูก)
            need_scan = (
...
# ==============================================================================
# Plot map (วาง Marker ตามตำแหน่งจริงบนกำแพงด้วย offset_m ซ้ายบวก/ขวาลบ)
# ==============================================================================
def plot_map_with_walls(graph, blocked, path, marker_map, filename="maze_map.png"):
    ...
    # Marker บนกำแพงด้วย offset_m (หน่วยกริด = offset/GRID_SIZE_M)
    for name, hits in (marker_map or {}).items():
        for finding in hits:
            (gx, gy) = finding['grid_pos']
            wall = finding['wall']
            offset = finding['offset_m'] / GRID_SIZE_M  # แปลงเป็นหน่วย "ช่อง"
